...
/**
 * Merge the split 613281 line items into one per SO.
 * Migrates picks from deleted line items to the kept one.
 *
 * Usage:
 *   node scripts/merge-613281.mjs              # Dry run
 *   node scripts/merge-613281.mjs --execute     # Apply changes
 */
...
async function main() {
  ...
  for (const soNumber of SO_NUMBERS) {
    ...
    const { data: items } = await supabase
      ...
      .order('qty_per_unit', { ascending: false });
